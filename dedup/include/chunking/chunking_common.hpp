...
#include <sstream>
#include <istream>
#include <unordered_set>  // Correct header for unordered_set
...
class Chunking_Technique{
    ...
    protected:
         ...
         */
        int64_t create_chunk(std::vector<std::string>& hashes,
                     char* data,
                     uint64_t buffer_end);  // old one for compatibility

        int64_t create_chunk(std::vector<std::string>& hashes,
                     char* data,
                     uint64_t buffer_end,
                     std::unordered_set<std::string>& seen_hashes);  // new
